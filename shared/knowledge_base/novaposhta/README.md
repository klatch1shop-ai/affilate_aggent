...
### ТТН у Rozetka і статус 61 (власник, 13.09: «додавай номер на розетку та роби статус»)

* `PATCH /orders/{id} {"status": 61, "ttn": "<ТТН>"}` — перевірено на
  #905931436 (зі статусу 26): `success`, читання показало статус 61
  «Заплановано передачу перевізникові» і ТТН у `delivery.declaration_number`.
  Статус 61 є в `status_available` зі статусу 26. Функцію агента `set_ttn()`
  (`/orders/add-ttn`, `delivery_service_id: 1`) жодного разу не викликали —
  не перевірена.
* `np_create_ttn.py` після створення робить це сам (`rozetka_ttn()`), якщо
  61 доступний, і перевіряє читанням; результат — рядком у Telegram.
* Агент замовлень на статус 61: для NOIRE-замовлень гілка NOIRE завершується
  раніше, ніж блок «статус 61 → Excel до Carvol», тож NOIRE до Carvol не
  потрапить; `manual_review` з ТТН сторож лише позначає.

**Пастка — друк блокує зміни:** після отримання наклейки
(`printMarking100x100`) `InternetDocument.update` відповідає «Document is
already printed» (13.09 на 20451535086672 — коробку вже не додати; на ціну
не впливає: 2 кг фактичної ваги > об'ємної). Тому коли власник має додати
післяплату вручну — скрипт наклейку не друкує, у Telegram лише текст.
