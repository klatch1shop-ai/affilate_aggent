...
Оновлено: 2026-06-28 (сесія 2). Читати на початку кожної сесії разом з CLAUDE.md.
...
| **Єпіцентр — TOPTUL** | 🔜 ЗАЛИШОК | Чернетки+заблоковані видалено 28.06.2026. ~700 опублікованих без продажів → видалити вручну |
...
- [x] Crontab: rozetka_github_sync змінено з `0 * * * *` (щогодини) → `15 7 * * *` (щодня о 7:15, після feed_sync)
...
- [x] **Чернетки і заблоковані TOPTUL видалено вручну** (28.06.2026, власник через кабінет)
- [ ] **~700 опублікованих TOPTUL без продажів за 2 тижні** — запланований ручний видалення власником
      > Видалення через Merchant API НЕМОЖЛИВЕ (підтверджено). Тільки вручну через кабінет.
      > ⚠️ delete_products в epicentr_mcp.py НЕ ІСНУЄ — не реалізовано, не використовувати
...
### Зроблено ✅
- [x] **`tools/epicentr_competitor_scraper.py`** — HTTP-парсер epicentrk.ua (2026-06-28)
  - Пошук за запитом (40 карток/сторінку) + парсинг повної картки (ціна, specs, фото)
  - `BaseHttpScraper` — реюзабельний базовий клас для SSR-сайтів без Cloudflare
  - Зберігає в `competitor_prices` (таблиця розширена: title/vendor/specs_json/photos_json)
  - Задокументовано: `shared/knowledge_base/competitor_scraping.md`
  - Протестовано: Teyes CC3 — 26950 грн, 19 specs, 5 фото, categoryId=2866 ✅
...
- [ ] Адаптувати для Rozetka (потрібен Playwright через Cloudflare — `playwright_base.py`)
- [ ] Адаптувати для Prom (те ж саме)

### Технічний стек
- HTTP (`requests` + `BeautifulSoup`) — для epicentrk.ua (SSR, без CF) ✅
- Playwright (`playwright_base.py`) — для Rozetka/Prom (Cloudflare) — ще не зроблено
- Claude API (claude-haiku-4-5) для SEO переписування
- PostgreSQL таблиця `competitor_prices`
