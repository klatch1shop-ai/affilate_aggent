...
## Рішення менеджера (21.09.2026)

Зі шести пропозицій Codex беру **три**, решту відкладаю з причинами.

| # | Пропозиція | Рішення | Чому |
|---|---|---|---|
| 1 | бюджет виходу до Ollama | ✅ **зроблено** | `call_ollama` приймав `max_tokens` і не передавав його — параметр брехав. `shared/utils/llm_router.py`, тести `tests/test_llm_router_limits.py` |
| 2 | облік справжніх токенів | ✅ **зроблено** | без нього жодну обіцянку економії не перевірити. Тепер `ask()` повертає `tokens`, журнал пише `tokens_in/out/total`. Перевірено живим викликом: 13 вхідних, 2 вихідних |
| 4 | повний лог окремо, структуровані помилки | ✅ **зроблено інакше** | замість правки бойового `tools/llm_delegate.py` зробив `scripts/qtest.py`: повний вивід у `logs/last_test_run.txt`, у контекст — один рядок або перелік FAILED. Це б'є по **моєму** контексту, а він у нас найдорожчий |
| 3 | обмежити історію XML-помічника | ⏸ **відкладено** | `tools/ai_xml_generator.py` — бойовий скрипт, зараз не в щоденному вжитку. Правити його заради гіпотетичної економії без вимірювання — саме те, від чого Codex застерігав. Повернутись, коли зʼявляться числа з п. 2 |
| 5 | посторінкове читання в MCP | ⏸ **відкладено** | середній ефект, зачіпає `shared/mcp_servers/` — окреме завдання з тестами |
| 6 | прибрати подвійні читання в `route_task` | ❌ **не робимо** | сам Codex написав: виграш у I/O, **не в токенах**. Наша мета зараз — токени |

**Що змінилось у щоденній роботі:** замість `pytest tests/ -q` тепер
`venv/bin/python scripts/qtest.py` — один рядок замість повного виводу,
а коли падає — перелік FAILED і шлях до повного логу.

**Як перевіримо, що стало краще:** через тиждень порахувати з `logs/llm_router.jsonl`
суму `tokens_total` на одну успішну задачу. Доти жодних відсотків не обіцяємо.
