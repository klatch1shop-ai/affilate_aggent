...
- `status` лишається `'open'`, **поки не записані всі** звірки. Коли записана остання —
  `status` стає `verdict` звірки з **найбільшим `stage`** (саме вона підсумкова).
  Це навмисно: інакше гіпотеза зникла б із `due` після першої ж звірки.
- Повертає оновлений запис.

### `open_items()` — гіпотези зі `status == 'open'` (тобто із незаписаними звірками), новіші вище.
