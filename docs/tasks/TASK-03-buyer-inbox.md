...
1. **`--allow`** — усі 8 нових файлів із §3 і `tg_dispatcher/ai_brain/commands.py`.
   Звіт `docs/reports/TASK-03-report.md` дозволено завжди.
2. **`pageCount > max_pages` — неповний обхід, і це теж «помилка» для курсора.**
   Слушне зауваження: інакше повідомлення з необійдених сторінок пропадуть. Правило:
   якщо `pageCount > max_pages`, обійти перші `max_pages` сторінок, обробити їх
   як звичайно (нові повідомлення записати й повернути в `new`), **курсор не зсувати**
   і додати в `errors` рядок, що містить обидва числа (скільки сторінок обійдено і
   скільки їх є). Дубліти на наступному запуску відсіє `store.add_new`.
