...
## Файли
Створити `research/rz_delivery_fit.py` (**не** в `tools/` — там бойові скрипти, каталог захищений). Лише стандартна бібліотека **плюс** уже перевірений
...
{'ready': [...], 'too_big': [...], 'unknown_dims': [...], 'no_price': [...],
 'overpriced': [...], 'totals': {...}, 'profit_sum': <float>}
```
...
- **кошик — перший, що підійшов**, саме в цьому порядку:
  ...
  5. `ready` — решта (у них `ok is True`).

  **Кошика «мала маржа» нема навмисно:** `price_item` отримує `min_profit` і сам піднімає
  ціну так, щоб прибуток був не меншим. Тому прибуток нижчий за `min_profit` при наявній ціні
  неможливий; умова `profit >= min_profit` у `ok` лишається як запобіжник, не як кошик.
...
- `totals` — `{назва кошика: кількість}` по п'ятьох кошиках, плюс `'all'` (усі товари)
  і `'raised'` — скільки з `ready` мають `status == 'raised_for_margin'`, тобто ціну довелося
  підняти вище РРЦ, щоб покрити опт, комісію й доставку;
...
Завеликі: 1 · Дорожчі за конкурентів: 1 · Без ціни: 1 · Без габаритів: 1
Ціну підняли вище РРЦ заради беззбитковості: 0
⚠️ Без габаритів не можна вважати прохідними — це 1 товар
• GAAR1002 — 976 ₴, прибуток 265.32 ₴
• T825051 — 1141 ₴, прибуток 203.00 ₴
```
- другий рядок лише якщо `ready` непорожній; рядок «Ціну підняли…» — лише якщо `totals['raised']`
  більший за нуль; рядок `⚠️` лише якщо `unknown_dims` непорожній;
