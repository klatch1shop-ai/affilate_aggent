---
name: "prom"
description: "Prom.ua: оновлення цін, price_engine, генерація XML каталогу, CPA-комісії, замовлення. Довідкові файли поруч: SKILL-11-api, SKILL-13-prom-seo (ключові слова й видача), SKILL-14-seo-research, SKILL-31-prom-search (видимість картки за prom_id, які поля враховує пошук)."
---
...
Винесено в окремий файл: [SKILL-11-api.md](SKILL-11-api.md)

Коротко, щоб не наступити двічі:

- **Категорію через API змінити НЕМОЖЛИВО.** `group_id`, `portal_category_id`,
  `category_id`, `category` — усі чотири повертають `Unknown field`, попри те
  що документація стверджує протилежне. Виправлення категорій — лише через
  фід або вручну в кабінеті.
- `POST /products/edit_by_external_id` **не приймає `external_id`** — вимагає
  `id`, як і `/products/edit`.
- HTTP 200 ще не означає успіх: перевіряй непорожній `processed_ids`.
- `group` (папка магазину) ≠ `category` (категорія каталогу). Ліве меню
  кабінету показує групи, тому призначену Promʼом категорію там не видно.
- Токени на ноуті й на сервері різні — перевіряй ту копію `.env`, з якої
  піде виклик.
