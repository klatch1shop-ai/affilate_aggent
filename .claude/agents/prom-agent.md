...
Prom захищений Cloudflare — HTTP-парсинг частково заблоковано. Для скрейпінгу цін конкурентів використовуй `agents/scraper/market_price_analyzer.py` (Playwright sync, prom.ua/ua/search).
За зразком базового класу: `tools/epicentr_competitor_scraper.py` + `shared/knowledge_base/competitor_scraping.md`. Результати пишуться в `competitor_prices` (marketplace='prom').
