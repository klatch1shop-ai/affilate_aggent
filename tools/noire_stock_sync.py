...
def regenerate_feed() -> dict:
    ...
    try:
        ...
        # Останній рубіж перед валідатором: звірка valuecode із довідником
        # КОЖНОЇ категорії. Імпорт 04.09.2026 відхилив значення у 319 картках —
        # «класична конусна» у «Формі» лубриканта, де довідник має лише
        # кругла/овальна/прямокутна. Валідатор такого не бачить: він перевіряє
        # наявність атрибута, а не те, чи значення взагалі існує в категорії.
        try:
            va = subprocess.run([py, os.path.join(BASE_DIR, 'tools',
                                                  'epicentr_valuecode_audit.py'),
                                 '--src', tmp, '--out', tmp],
                                capture_output=True, text=True, timeout=1200)
            if va.returncode == 0:
                last = [l for l in (va.stdout or '').splitlines() if 'ХИБНИХ' in l]
                logger.info('Звірка valuecode: ' + (last[0] if last else 'ok'))
            else:
                logger.error('Звірка valuecode не вдалась, фід як є: '
                             f'{(va.stderr or "")[-250:]}')
        except Exception as exc:
            logger.error(f'Звірка valuecode впала ({exc}), фід як є')
...
def main():
    ...
    if a.quick:
        ...
        #
        # Запобіжник: публікація фіду Єпіцентру — рішення власника. Повторний
        # імпорт фіду стирає категорії й характеристики карток у статусі
        # «Наповнення контентом», тобто знищує роботу категорійного менеджера.
        # Rozetka і Prom нижче НЕ під запобіжником: їх публікація щогодинна,
        # узгоджена, і фід має лишатись свіжим.
        from shared.utils.consent import require as consent_require
        consent_require('epicentr_publish',
                        'перезібрати noire_epicentr.xml і запушити на GitHub')
        feed = regenerate_feed()
        if not feed.get('ok'):
