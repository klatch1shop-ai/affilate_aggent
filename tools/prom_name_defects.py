...
POSITIVE = [
    ...
    ('Виброяйце Dorcel Secret Delight Gold з пультом ДК', 'ru'),
...
def classify_name(name, freq, uk):
    ...
    for w in LEX.ru_words(text):
        out.append(('ru', w, ''))
    # російські основи, яких немає в українській: «вибро-» (укр. «вібро-»),
    # «массаж-» (укр. «масаж-») — словник таких слів не знає, а для
    # одруківки вони надто часті («Виброяйце» у 3 назвах, 13.09.2026)
    for w in re.findall(r"(?<![\wʼ’'-])(?:вибро\w*|массаж\w*)", text.lower()):
        out.append(('ru', w, ''))
