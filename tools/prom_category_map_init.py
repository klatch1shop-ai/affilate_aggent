...
MAP = [
    ...
    ('9452', '161003', r'лубрикант|лубикант|змазк|гель-?змазк|\bglide\b|\blube\b', 0.95,
    ...
    # ── Уточнення за назвою, знайдені звіркою з фактичним станом кабінету ──
    # Prom перевизначив 853 з 5495 товарів (15,5%). Там, де його вибір
    # точніший за наш, правило переносимо в мапінг, а не покладаємось на те,
    # що алгоритм поводитиметься так само наступного разу.
    ('9476', '161018', r'кліторальн|вакуумн.{0,14}стимулятор|satisfyer|womanizer'
                       r'|повітрян.{0,10}хвил', 0.95,
     'Екстендери та помпи → Вакуумні стимулятори. Satisfyer Pro/Penguin — '
     'кліторальні стимулятори, а не помпи для члена. Prom правий, 218 товарів.'),
    ('9466', '161020', r'секс-?машин|sex ?machine', 0.95,
     'Вібратори → Секс-машини за назвою. У постачальника окремої категорії '
     'немає, машини лежать серед вібраторів. Prom правий, 23 товари.'),
    ('9472', '161020', r'секс-?машин|sex ?machine', 0.95,
     'Мастурбатори → Секс-машини за назвою (Kiiroo Keon). Prom правий, 6.'),
    ('9466', '161018', r'вакуум|повітрян.{0,10}хвил', 0.90,
     'Вібратори → Вакуумні стимулятори, коли в назві вакуум. Prom правий, 24.'),
    ('9450', '16221301', r'масажн.{0,12}(олі|крем|піна|гель|засіб)|для масажу', 0.90,
     'Масажна косметика → «Масажні косметичні засоби» (Краса та здоровʼя). '
     'Точніше за «Збуджуючі засоби», куди ми клали через брак категорії '
     'у гілці інтимних товарів. Prom правий, 64 товари.'),
    ('9632', '16221301', None, 0.90,
     'Олія для еротичного масажу → Масажні косметичні засоби. Те саме.'),
    ('9460', '170702', r'брелок|брелк', 0.95,
     'Еротичні приколи → Брелоки. Prom правий, 31 товар.'),
    ('9526', '15131004', r'менструальн', 0.95,
     'Аксесуари до іграшок → Менструальні чаші. Prom правий, 13 товарів.'),
    ('9448', '15131004', r'менструальн', 0.95,
     'Догляд за іграшками → Менструальні чаші. Те саме.'),
    ('9550', '161217', r'спринцівк|анальний душ|douche', 0.95,
     'Анальний душ → Спринцовки. Prom правий, 7 товарів.'),
    ('9526', '161217', r'спринцівк|анальний душ|douche', 0.95,
     'Те саме для аксесуарів.'),
    ('9526', '1507', r'^батарейк|батарейка\b', 0.95,
     'Аксесуари → Батарейки. Prom правий, 3 товари.'),
    ('9526', '63719', r'кабел|зарядк|charging cable|адаптер bluetooth', 0.90,
     'Аксесуари → Кабелі для електроніки. Prom правий, 4 товари.'),
    ('9526', '70811', r'вебкамер|веб.?камер|webcam', 0.95,
     'Аксесуари → Веб-камери (Lovense WebCam). Prom правий, 2 товари.'),
    ('9526', '63010', r'тример|епілятор|shaver', 0.90,
     'Аксесуари → Жіночі епілятори та електробритви. Prom правий, 2 товари.'),
    ('9452', '330619', r'інтимн.{0,10}гігієн|для інтимних зон|intime balance', 0.90,
     'Збуджуючі → Засоби для інтимної гігієни. Prom правий, 7 товарів.'),
    ('9452', '330623', r'сіль для ванн|для ванни', 0.90,
     'Збуджуючі → Засоби для прийняття ванн. Prom правий, 5 товарів.'),
    ('9452', '330622', r'гель для душу|body wash', 0.90,
     'Збуджуючі → Засоби для душу. Prom правий, 3 товари.'),
    ('9452', '331502', r'після гоління|для гоління|shav', 0.90,
     'Збуджуючі → Косметика для гоління. Prom правий, 2 товари.'),
    ('9478', '16102103', r'кульк|balls', 0.95,
     'Вагінальні кульки та тренажери → Вагінальні кульки, коли в назві кульки. '
     'Наше правило по «тренажер» відправляло їх у кегеля. Prom правий, 6.'),

    # ── Група В: наші явні промахи, де вибір Prom обґрунтований ────────────
    # Тут категорія постачальника описує полицю, а не сам товар: у «Меблях»
    # лежать фіксатори, у «Збуджуючих засобах» — БДСМ-свічки тощо.
    ('9452', '161007', r'низькотемпературн|бдсм.{0,12}свічк|sm low temperature', 0.90,
     'Збуджуючі → БДСМ-іграшки: низькотемпературні свічки Fetish Tentation — '
     'це БДСМ-реквізит, а не косметика. 13 товарів.'),
    ('9484', '161007', r'уретральн|sperm stopper|penis plug|катетер', 0.90,
     'Анальні пробки → БДСМ-іграшки: уретральні стимулятори Sinner Gear. 12.'),
    ('9578', '161007', r'фіксатор|розтяжк|прив.язі|straps for bed|бондаж', 0.90,
     'Меблі для сексу → БДСМ-іграшки: фіксатори рук і ніг та розтяжки на '
     'ліжко — це бондаж, а не меблі. 8 товарів.'),
    ('9526', '161011', r'насадка для вібромасажер|headgear|насадка.{0,14}вібратор', 0.90,
     'Аксесуари → Вібратори: змінні насадки для вібромасажера Leten. 7.'),
    ('9472', '161003', r'\bnuru\b|гель для .{0,8}масажу|body-?to-?body', 0.90,
     'Мастурбатори → Лубриканти: гелі для NURU-масажу Shunga. 6 товарів.'),
    ('9466', '161002', r'подарунков.{0,12}набір.{0,40}(крапл|гель|олі|афродизіак)', 0.85,
     'Вібратори → Збуджуючі засоби: подарункові набори, де основа — краплі '
     'для клітора чи гелі, а не сама іграшка. 6 товарів.'),
