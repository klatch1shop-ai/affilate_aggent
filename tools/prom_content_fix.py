    ...
    for o in rows:
        ...
        if de is not None:
            ...
            if vendor and vendor.lower() not in plain[:300].lower():
                ...
                # Без кирилиці в дужках — рішення власника 12.09.2026, див. нижче.
                html = (f'<p><b>{name.split(",")[0]}</b> від бренду {vendor}'
                        + '.</p>' + html)
                did['бренд винесено в перший абзац опису'] += 1
            # Кириличний бренд у дужках («Art of Sex (арт оф секс)») більше НЕ
            # вставляємо — рішення власника 12.09.2026. Вимога з консультації
            # 02.09 («у описі має бути бренд кирилицею») спиралась на те, що
            # опис бере участь у пошуку; тест 12.09 (SKILL-31) показав, що
            # слова опису картку у видачі не утримують, а покупцю дужка з
            # транслітерацією виглядає дивно. Функцію лишено для історії.
            c1 = False
            if c1:
