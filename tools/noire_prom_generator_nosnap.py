...
try:
    from homoglyph import fix_text as _fix_homoglyphs
except Exception:  # модуль лежить поруч; без нього — текст як є
...
def dehomo(t: str, st=None, what: str = '') -> str:
    ...
    12.09.2026). Для пошуку це інше слово: «свечa» не збігається з «свеча».
    Правила напрямку — у tools/homoglyph.py (там же для фіду TOPTUL)."""
    if not t or _fix_homoglyphs is None:
        return t
    new, hits = _fix_homoglyphs(t)
    # Злиплі слова на межі абеток: «пробкаWooomy», «членаArt», «Honeyпросто»
    # (3 на фід). Пошук бачить одне невідоме слово замість двох.
    new2 = _GLUE_CL.sub(r'\1 \2', _GLUE_LC.sub(r'\1 \2', new))
    if (hits or new2 != new) and st is not None:
        st[f'мішане написання виправлено ({what})'] += 1
    return new2


_GLUE_CL = re.compile(r'([а-яіїєґ]{3,})([A-Z][a-z]{2,})')
_GLUE_LC = re.compile(r'([a-z]{3,})([а-яіїєґ]{3,})')
# латинська літера → кириличний двійник (однакові на вигляд)
_TWIN = dict(zip('aceiopxyACEHIKMOPTXB', 'асеіорхуАСЕНІКМОРТХВ'))
_UNTWIN = {c: l for l, c in _TWIN.items()}


def brand_twins(t: str, brand: str) -> str:
    """Бренд картки, набраний кириличними двійниками: «Кііrоо» замість
    Kiiroo (7 описів, 12.09.2026). homoglyph.py такий токен не рухає —
    три кириличні літери поспіль спрацьовують як запобіжник проти «Vрес».
    Тут напрямок відомий напевно: це назва бренду з <vendor>."""
    if not t or not brand or not re.fullmatch(r'[A-Za-z][A-Za-z0-9 .&\'-]*', brand):
        return t
    pat = ''.join(f'[{c}{_TWIN[c]}{_TWIN.get(c.swapcase(), "")}{c.swapcase()}]'
                  if c in _TWIN else re.escape(c) for c in brand)

    def sub(m):
        s = m.group(0)
        return ''.join(_UNTWIN.get(ch, ch) for ch in s)
    return re.sub(rf'(?<!\w){pat}(?!\w)', sub, t, flags=re.I)
...
def ru_keywords(kw_ua: str, ru_row: dict, vendor: str, category: str = '',
    ...
    if (vendor or '').strip().lower() in NO_BRAND:
        vendor = ''
    name_ru = dehomo((ru_row or {}).get('name_ru') or '')
    if not name_ru or not (ru_row or {}).get('is_ru'):
        return ''
    # Дефолт ('', 'm', '') виглядає як тихий відмов, але ним НЕ є — перевірено
    # 08.09.2026. Рід використовується рівно в одному місці нижче
    # (`_RU_MATERIAL[material][_RU_GENDER[gender]]`), і та гілка закрита
    # умовою `and single`. Коли категорії немає в довіднику, `single` порожній,
    # тож 'm' не доходить до жодного шаблону — це мертве значення.
    # Заміряно на фіді: 71 товар у 14 категоріях без запису в довіднику мають
    # у СЕРЕДНЬОМУ БІЛЬШЕ російських фраз (8.68 проти 8.47), порожніх нема
    # взагалі — `_fill()` добирає тегами категорії.
    # Не чіпати без нового заміру: спроба «виправити» це 08.09 ледь не внесла
    # NameError у продакшен.
...
def generate(out_file=OUT, limit=None):
    ...
    for key, items in groups.items():
        ...
        # До 15.09.2026 тут категорію підміняв prom_actual_category — одноразовий
        # знімок API від 16.08 для ВСІХ товарів (не лише перенесених Prom), що
        # не оновлювався. Через нього жодна зміна зіставлення з 16.08 не діяла
        # на товари, які тоді існували (293 розбіжності). Рішення власника
        # 15.09: знімок прибрати — категорію дає лише зіставлення. Лічильник
        # лишено, щоб бачити, скільки карток розходяться зі знімком.
        actual = ACTUAL_CAT.get(head['sku'])
        if actual and actual != pid:
            st['зіставлення ≠ знімок 16.08 (діє зіставлення)'] += len(items)
        if not pid:
        ...
        for p in items:
            ...
            name = brand_twins(name, vend_clean)
            ...
            name_ru = brand_twins(dehomo(name_ru, st, 'назва рос.'), vend_clean)
            ...
            desc_ru = brand_twins(dehomo(desc_ru, st, 'опис рос.'), vend_clean)
            if desc_ru:
            ...
            kw = dehomo(keywords.get(sku, ''), st, 'ключі укр.')
            # Категорія для рос. ключів і тегів — з фіду, а не зі знімка
            # таблиці ключів (02.09): там 277 карток у старій категорії —
            # масажні свічки й олії як «Збуджуючі засоби» (рос. ключ
            # «возбуждающее средство» на свічці), LELO SONA як «Вакуумні
            # помпи» (13.09.2026). Знімок — лише запасний варіант.
            _fc = (cat_names.get(pid) or '').strip()
            cat_ua = _fc if _fc in _load_ru_cat() else kw_cats.get(sku, '')
