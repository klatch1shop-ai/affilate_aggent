...
def main():
    ...
    if a.exclude_existing:
        import json as _j
        # Спершу тут стояв виняток: картку в роботі (draft/enrich) файл мав
        # **доповнити**. Перевірено 04.09.2026 — Єпіцентр таке відхиляє:
        # «Оновлення товарів не доступне», 6 карток із 6. Тож для партії нових
        # товарів виключаємо ВСЕ, що вже є в кабінеті: інакше половина файлу
        # піде в помилки й засмітить звіт імпорту.
        LOCKED = None   # None = виключати будь-який наявний артикул
        cab = _j.load(open(a.exclude_existing, encoding='utf-8'))
        existing = {(x.get('sku') or '').strip() for x in cab
                    if LOCKED is None or (x.get('status') or '') in LOCKED}
        print(f'у кабінеті: {len(cab)} | виключено з файлу: {len(existing)}')
    merged = None
    offers_el = None
    kept = dropped = 0
    for path in a.src:
