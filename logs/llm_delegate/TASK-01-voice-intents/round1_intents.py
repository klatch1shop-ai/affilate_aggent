    ...
    for intent, keywords in _INTENT_KEYWORDS.items():
        ...
        if score > best_score:
            ...
            if score >= 2:
                ...
                # Перевіряємо, чи це лише загальне слово (наприклад, "ціни")
                matched_kw = next(kw for kw in keywords if kw in normalized)
                if any(gen in matched_kw for gen in _PRICE_GENERIC_WORDS) and len(matched_kw.split()) == 1:
                    best_confidence = 0.5
                else:
                    best_confidence = 0.8
            else:
